...
class FixIndexForPublicTimelineSpeeds < ActiveRecord::Migration[7.1]
  ...
  def up
    add_index :statuses, [:id], name: :index_statuses_local_20240108, algorithm: :concurrently, order: { id: :desc }, where: '(local OR (uri IS NULL)) AND deleted_at IS NULL AND visibility IN (0, 10, 11) AND reblog_of_id IS NULL AND ((NOT reply) OR (in_reply_to_account_id = account_id))'
  ...
  def down
    ...
    remove_index :statuses, name: :index_statuses_local_20240108
